...
if(isset($_GET['auth_token'])){
  ...
  $verifyAccount = $connectToDb->query('SELECT verified, verification_key FROM newUsers WHERE verified =0 AND verification_key="'.$auth_token.'"');
  if($verifyAccount -> num_rows == 1){
    $updateAccount = $connectToDb->query('UPDATE newUsers SET verified=1 WHERE verification_key="'.$auth_token.'"');
    if($updateAccount){
      header('Location: login.php?accountVerified');
    }
    else{
      echo $connectToDb->error;
    }
  }
  else{
    echo '
...
<body>
  <div class="layout" style="position: relative; left: -650px;">
		<div class="title">Account Already Verified <i class="fas fa-check-circle fa-sm text-success"></i></div>
		<div class="text icon_500">No further action is necessary</div>
		<div class="buttons">
      <a class="text_buttons_intro text-light" href="login.php" style="text-decoration: none; font-weight: 900;">Go ahead and login</a>
		</div>
  </div>
</body>
</html>
